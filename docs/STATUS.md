...
**Живая проверка Этапа 3** ждёт ключ поисковика: в `.env` пусты `TAVILY_API_KEY`,
`EXA_API_KEY`, `FIRECRAWL_API_KEY`. Нужен хотя бы один (основной — Tavily). Код
реализуется независимо; без ключа `run_web_search` отдаёт пустой результат.
