...
**Последнее обновление**: 2026-07-02 (закрыт Этап 2 — базовый ИИ-ответ, tag stage-2-done)
**Текущий этап**: Этап 3 — ИИ-агент с веб-поиском (Tavily + капы) (☐ не начат)
**Следующий шаг**: Взять Этап 3 в работу — инструмент `web_search` (Tavily основной, Exa/Firecrawl запасные), tool-calling петля на `gemini-2.5-flash` с капом ≤3–4 шага + лимит токенов, статусы «ищу информацию…», ответ со ссылками, списание 1 запрос за весь поиск. Дизайн — [`AGENT_PIPELINE.md`](AGENT_PIPELINE.md); критерии — [`07_ROADMAP.md`](07_ROADMAP.md#этап-3--ии-агент-с-веб-поиском-tavily--капы).
...
Пока нет. Этап 2 закрыт, ждём отмашку на старт Этапа 3.
